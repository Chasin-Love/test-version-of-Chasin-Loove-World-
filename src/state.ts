...
import { delLocalPayload, delPayload, getPayload, putLocalPayload } from './storage/indexedDB';
import { procPalette, procRadius } from './storage/procedural';
import { createInitialSeed, seedBodies } from './storage/seeds';
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed && Array.isArray(parsed.bodies) && parsed.bodies.length) {
        ...
        efsHeal(parsed.efs, parsed.vault);
...
export const actions = {
  ...
  renameReality(realityId: string, name: string) {
    if (!name.trim()) return;
    actions.updateRealityMeta(realityId, { name: name.trim() });
  },

...
export function FileManager({ onOpen, onImport, onLock }: {
  ...
  const kidsOf = (dirId: string) =>
    childrenMap.get(dirId) ?? { dirs: [] as VfsNode[], fileIds: [] as string[] };
  ...
  const cwdFiles = useMemo(() => {
    if (!flat) {
      return kidsOf(cwdId).fileIds
        .map((nid) => fileById.get(vfs.nodes[nid]?.fileId ?? ''))
  ...
  }, [childrenMap, cwdId, flat, fileById, vfs.nodes]);
            ...
            <div className="max-h-27.5 overflow-y-auto thin-scroll space-y-1">
              ...
              {vfs.shadows.map((s) => (
                <div key={s.id} className="flex items-center gap-2 font-mono text-[9px] text-slate-soft bg-void/40 border border-line/40 px-2 py-1">
                  <span className="truncate flex-1" title={s.description ?? ''}>{s.name}</span>
                  ...
                  <button onClick={() => actions.efsDeleteShadow(s.id)} className="text-red-300/70 hover:text-red-300">✕</button>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="fm-tile-grid pb-2">
            {sortedDirs.map((d) => (
              <div key={d.id} {...rowProps(d, null)}
                ...
                <span className="fm-tile-sub mt-1">{kidsOf(d.id).dirs.length + kidsOf(d.id).fileIds.length} item{(kidsOf(d.id).dirs.length + kidsOf(d.id).fileIds.length) === 1 ? '' : 's'}</span>

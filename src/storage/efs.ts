...
export function efsHeal(vfs: VfsState, vault: VaultFile[]): number {
  ...
  for (const f of vault) {
    ...
    if (existing) {
      /* repair drifted metadata only */
      if (existing.name !== f.name) existing.name = f.name;
      if (f.dirId && existing.parentId !== f.dirId && vfs.nodes[f.dirId]) existing.parentId = f.dirId;
      continue;
    }
    const parent = f.dirId && vfs.nodes[f.dirId] ? f.dirId : EFS_ROOT;
    f.dirId = parent;

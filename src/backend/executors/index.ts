 ...
 * 
 * Orchestrates sandboxed client-side execution for any data type:
 *  - ISO 9660 Disc Images (.iso)  → Virtual mount, volume descriptor, directory tree & file extractor
 *  - HTML/CSS/JS Applications     → Sandboxed CSP-restricted iframe runtime with live asset blob rewriting
 *  - JavaScript / TypeScript      → Isolated Web Worker with piped virtual console
 *  - Python 3                     → Pyodide WebAssembly runtime (CPython 3.12)
 *  - PDF Documents                → High-fidelity sandboxed viewer
 *  - Archives (.zip, .tar, etc.)  → Real directory extraction & in-memory runner
 */
...
export type { ZipEntry } from '../storage/zip';
export * from './isoExecutor';

export function detectRunner(file: VaultFile): RunnerKind | null {
  const ext = file.name.split('.').pop()?.toLowerCase() ?? '';
  
  // ISO 9660 Disc Images
  if (ext === 'iso' || file.mime === 'application/x-iso9660-image' || file.mime === 'application/x-cd-image') {
    return 'iso';
  }
  ...
  // Web Applications & Interactive Modules
  if (ext === 'html' || ext === 'htm' || file.mime === 'text/html') return 'web-app';
  
  // JavaScript / Modules
  if (ext === 'js' || ext === 'mjs' || file.mime === 'text/javascript' || file.mime === 'application/javascript') {
    return 'javascript';
  }
...
export async function readArchiveListing(blob: Blob): Promise<ZipEntry[]> {
  const buf = await blob.arrayBuffer();
  return readZipEntries(buf);
}

/** Extracts one entry as a Blob (for download or import into the vault). */
export async function extractArchiveEntry(blob: Blob, entry: ZipEntry): Promise<Blob> {
  const buf = await blob.arrayBuffer();
